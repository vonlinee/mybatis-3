...
class BaseExecutorTest extends BaseDataTest {
  ...
  void shouldInsertNewAuthorWithBeforeAutoKey() throws Exception {
    ...
    try {
      ...
      int rows = executor.update(new MapperUpdate(insertStatement, author));
      ...
      if (author.getId() != BatchExecutor.BATCH_UPDATE_RETURN_VALUE) {
        List<Author> authors = executor.query(new MapperQuery(selectStatement, author.getId()));
  ...
  void shouldInsertNewAuthor() throws Exception {
    ...
    try {
      ...
      MappedStatement selectStatement = ExecutorTestHelper.prepareSelectOneAuthorMappedStatement(config);
      int rows = executor.update(new MapperUpdate(insertStatement, author));
      List<Author> authors = executor.query(new MapperQuery(selectStatement, 99));
      executor.flushStatements();
      executor.rollback(true);
      assertEquals(1, authors.size());
      assertEquals(author.toString(), authors.get(0).toString());
  ...
  void shouldSelectAllAuthorsAutoMapped() throws Exception {
    ...
    try {
      ...
      List<Author> authors = executor.query(new MapperQuery(selectStatement, null));
  ...
  void shouldInsertNewAuthorByProc() throws Exception {
    ...
    try {
      ...
      MappedStatement selectStatement = ExecutorTestHelper.prepareSelectOneAuthorMappedStatement(config);
      executor.update(new MapperUpdate(insertStatement, author));
      List<Author> authors = executor.query(new MapperQuery(selectStatement, 97));
      executor.flushStatements();
      executor.rollback(true);
      assertEquals(1, authors.size());
      assertEquals(author.toString(), authors.get(0).toString());
  ...
  void shouldInsertNewAuthorUsingSimpleNonPreparedStatements() throws Exception {
    ...
    try {
      ...
      int rows = executor.update(new MapperUpdate(insertStatement, null));
      List<Author> authors = executor.query(new MapperQuery(selectStatement, 99));
      executor.flushStatements();
      executor.rollback(true);
      assertEquals(1, authors.size());
  ...
  void shouldUpdateAuthor() throws Exception {
    ...
    try {
      ...
      int rows = executor.update(new MapperUpdate(updateStatement, author));

      MappedStatement selectStatement = ExecutorTestHelper.prepareSelectOneAuthorMappedStatement(config);
      List<Author> authors = executor.query(new MapperQuery(selectStatement, 101));
      executor.flushStatements();
      executor.rollback(true);
      assertEquals(1, authors.size());
      assertEquals(author.toString(), authors.get(0).toString());
  ...
  void shouldDeleteAuthor() throws Exception {
    ...
    try {
      ...
      int rows = executor.update(new MapperUpdate(deleteStatement, author));
  ...
  void shouldSelectDiscriminatedPost() throws Exception {
    ...
    try {
      ...
      List<Map<String, String>> products = executor.query(new MapperQuery(selectStatement, null));
  ...
  void shouldSelect2DiscriminatedPosts() throws Exception {
    ...
    try {
      MappedStatement selectStatement = ExecutorTestHelper.prepareSelectDiscriminatedPost(config);
      List<Map<String, String>> products = executor
          .query(new MapperQuery(selectStatement, null).rowBounds(new RowBounds(2, 2)));
  ...
  void shouldSelectTwoSetsOfAuthorsViaProc() throws Exception {
    ...
    try {
      ...
      List<List<Author>> authorSets = executor.query(new MapperQuery(selectStatement, new HashMap<String, Object>() {
        {
          put("id1", 101);
          put("id2", 102);
        }
  ...
  void shouldSelectAuthorViaOutParams() throws Exception {
    ...
    try {
      ...
      executor.query(new MapperQuery(selectStatement, author));
  ...
  void shouldFetchPostsForBlog() throws Exception {
    ...
    try {
      ...
      List<Post> posts = executor.query(new MapperQuery(selectPosts, 1));
      executor.flushStatements();
  ...
  void shouldFetchOneOrphanedPostWithNoBlog() throws Exception {
    ...
    try {
      ...
      List<Post> posts = executor.query(new MapperQuery(selectPost, 5));
      executor.flushStatements();
      executor.rollback(true);
  ...
  void shouldFetchPostWithBlogWithCompositeKey() throws Exception {
    ...
    try {
      ...
      config.addMappedStatement(selectBlog);
      config.addMappedStatement(selectPost);
      List<Post> posts = executor.query(new MapperQuery(selectPost, 2));
      executor.flushStatements();
      assertEquals(1, posts.size());
      Post post = posts.get(0);
  ...
  void shouldFetchComplexBlogs() throws Exception {
    ...
    try {
      ...
      config.addMappedStatement(selectBlog);
      config.addMappedStatement(selectPosts);
      List<Blog> blogs = executor.query(new MapperQuery(selectBlog, 1));
      executor.flushStatements();
  ...
  void shouldMapConstructorResults() throws Exception {
    ...
    try {
      ...
      List<Author> authors = executor.query(new MapperQuery(selectStatement, 102));
      executor.flushStatements();
      executor.rollback(true);
      assertEquals(1, authors.size());
  ...
  void shouldClearDeferredLoads() {
    assertDoesNotThrow(() -> {
      ...
      try {
        ...
        executor.query(new MapperQuery(selectPosts, 1));

        Author author = new Author(-1, "someone", "******", "someone@apache.org", null, Section.NEWS);
        executor.update(new MapperUpdate(insertAuthor, author));

        MappedStatement selectAuthor = ExecutorTestHelper.prepareSelectOneAuthorMappedStatement(config);
        executor.query(new MapperQuery(selectAuthor, -1));
        executor.flushStatements();
        executor.rollback(true);
      } finally {
        executor.rollback(true);
  ...
  void createCacheKeyWithNull() {
    ...
    Executor executor = createExecutor(new JdbcTransaction(ds, null, false));
    Object cacheKey = executor.createCacheKey(mappedStatement, null, RowBounds.DEFAULT, boundSql);
